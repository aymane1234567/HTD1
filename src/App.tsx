...
function Heatmap({ habit }: { habit: Habit }) {
  ...
  return (
    <div>
      ...
      >
        {monthName} {now.getFullYear()} · China Standard Time
      </p>
      <div
        style={{
          display: "grid",
...
export default function App() {
  ...
  const [habits, setHabits] = useState<Habit[]>(() =>
    loadFromStorage("habits", INIT_HABITS)
  );
  ...
  const [nextHabitId, setNextHabitId] = useState(INIT_HABITS.length + 1);
  ...
  // Save to localStorage whenever tasks or habits change
  ...
  const toggleCheckin = (habitId: number) => {
    const key = currentKey(habits.find((h) => h.id === habitId)!.freq);
    setHabits((p) =>
      p.map((h) => {
          ...
          <>
            ...
            >
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={habitBarData} barSize={32} layout="vertical">
                  <XAxis
                    type="number"
                    domain={[0, 100]}
